...
// legacy/global keys, or when requireApiKey is off and no key was supplied).
async function validateGeminiNativeClientKey(request) {
  ...
  if (!settings.requireApiKey) return { tenantId };
...
async function forwardGeminiNativeRequest(request, body, model, action) {
  ...
  while (true) {
    const credentials = await getProviderCredentials("gemini", excludeConnectionIds, modelId, { tenantId });
