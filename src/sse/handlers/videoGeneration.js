...
// Returns { error } when auth fails, otherwise { tenantId } (null tenantId for
// legacy/global keys or when no key was supplied at all).
async function requireValidApiKey(request) {
  ...
  if (settings.requireApiKey) {
    ...
    if (!keyAuth || !keyAuth.isActive) return { error: errorResponse(HTTP_STATUS.UNAUTHORIZED, "Invalid API key") };
  }
  return { tenantId };
}
...
async function resolveVideoProvider(parsedBody, tenantId = null) {
...
export async function handleVideoCreate(request, action) {
  ...
  const resolved = await resolveVideoProvider(bodyInfo.parsed, tenantId);
...
export async function handleVideoGet(request, requestId) {
  ...
  const credentials = await getProviderCredentials(provider, null, null, { preferredConnectionId, tenantId });
  if (!credentials || credentials.allRateLimited) {
    return errorResponse(HTTP_STATUS.BAD_REQUEST, `No credentials for provider: ${provider}`);
  }
