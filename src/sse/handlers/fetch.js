...
export async function handleFetch(request) {
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      body,
      models: comboModels,
      handleSingleModel: (b, m) => handleSingleProviderFetch(b, m, request, apiKey, settings, tenantId),
      log,
  ...
  return handleSingleProviderFetch(body, providerInput, request, apiKey, settings, tenantId);
}

async function handleSingleProviderFetch(body, providerInput, request, apiKey, settings, tenantId = null) {
  const targetUrl = body.url;
  const format = body.format;
  const maxCharacters = body.max_characters;
