...
export async function handleSearch(request) {
  ...
  // (null for legacy/global keys) to scope combo + credential lookups downstream.
  ...
  // (truthy-tenantId convention: falsy/legacy tenantId keeps today's unfiltered getCombos())
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      body,
      models: comboModels,
      handleSingleModel: (b, m) => handleSingleProviderSearch(b, m, request, apiKey, settings, tenantId),
      log,
  ...
  return handleSingleProviderSearch(body, providerInput, request, apiKey, settings, tenantId);
}

async function handleSingleProviderSearch(body, providerInput, request, apiKey, settings, tenantId = null) {
  const query = body.query;
  ...
  while (true) {
    const credentials = await getProviderCredentials(providerId, excludeConnectionIds, null, { tenantId });
